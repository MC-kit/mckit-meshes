...
def test_reads_two_pages_from_contour_file(data, path, pages):
    path = data / path
...
def test_first_page_from_contour_file_has_3_lines(test_file):
...
def test_first_page_from_contour_file_meta_info_is_complete(test_file):
    with open(test_file) as fid:
        iterator = enumerate(rpf.read(fid))
        i, page = next(iterator)
        assert i == 0
        expected_date = dt.datetime(2018, 8, 24, 19, 4, 46)
        assert expected_date == page.date
        assert page.title == "hfs-reflectometer"
        expected_probid = dt.datetime(2018, 8, 24, 18, 20, 19)
